...
def scatter_upon_const_tensor_extra_check(match: Match):
    ...
    Pattern:
    1. Scatter is applied to a constant tensor
...
def scatter_upon_const_tensor_joint(
    ...
    def repl_fn(shape, background_val, dim, selector, val):
        ...
        # Broadcast selector to match the full tensor shape
        selector_expanded = selector.expand(shape)

        # Create a mask for where to scatter
        mask = selector_expanded == indices_view

        # Use torch.where to implement the scatter operation pointwise
